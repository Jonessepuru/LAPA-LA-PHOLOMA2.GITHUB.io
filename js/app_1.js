...
function renderTree(){
  ...
  if(parentSelect){
    parentSelect.innerHTML = '<option value="">Or choose from list (25 members)...</option>' + members.map(mem=>`<option value="${mem.id}">${mem.first} ${mem.last} ${mem.surname} (Gen${mem.gen}) - ${mem.lapa}</option>`).join('');
  }
